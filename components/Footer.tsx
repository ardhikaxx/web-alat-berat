import { business, navLinks, whatsappUrl } from "@/lib/constants";
    ...
    <footer className="bg-dark-bg text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="py-16 lg:py-20 grid md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8">
          ...
          <div className="lg:col-span-2">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <span className="font-bold text-lg block">{business.name}</span>
                <span className="text-dust text-xs">{business.tagline}</span>
              </div>
            </div>
            <p className="text-dust text-sm leading-relaxed max-w-md mb-6">
              {business.name} melayani rental, sewa, jual beli alat berat di{" "}
              {business.location} untuk kebutuhan konstruksi, pekerjaan tanah,
              proyek, dan berbagai kebutuhan lainnya.
            </p>
            <a
              href={whatsappUrl}
              target="_blank"
            ...
            <ul className="space-y-3">
              {navLinks.map((link) => (
                <li key={link.href}>
                  <a
                    href={link.href}
                    className="text-dust text-sm hover:text-yellow transition-colors"
                  >
                    {link.label}
                  </a>
                </li>
          ...
          <div>
            ...
            <ul className="space-y-3 mb-8">
              ...
              <li className="text-dust text-sm">{business.location}</li>
            </ul>
            ...
            <div className="flex gap-3">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="w-10 h-10 bg-dark-surface flex items-center justify-center text-dust hover:text-yellow hover:bg-dark-border transition-all"
        ...
        <div className="border-t border-dark-border py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-dust text-xs">{business.copyright}</p>
